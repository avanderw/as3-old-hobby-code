package avdw.demo.terrain {
	...
	public class ValueNoise extends Sprite {
		...
		private var detail:int = 12;
		private var roughness:Number = 0.65;
		private var func:int = 0;
		...
		private function generate(e:MouseEvent = null):void {
			heightmap = avdw.generate.terrain.dimension.two.ValueNoise.generate(size, detail, roughness, func);
			...
			// update text
			txt.text = "Value Noise" +
					   "\n---------------------------------------" +
			    "\nclick to generate" +
				"\ndetail (up/down):       " + detail +
				"\nroughness (left/right): " + (Math.round(roughness * 100) / 100) +
				"\ninterpolation (space):  ";
			
			switch (func) {
				case 0: txt.appendText("linear"); break;
				case 1: txt.appendText("cosine"); break;
				case 2: txt.appendText("cubic"); break;
			}
		}
